...
import { CAREER_MODE_LABELS, DEFAULT_CAREER_MODE, type CareerMode } from "@/lib/mode";
  ...
  return (
    <div key={activeMode} className="mode-transition-shell">
      <Hero
        ...
        modeLabel={CAREER_MODE_LABELS[activeMode]}
        metrics={content.glance.slice(0, 3)}
      />
      ...
      >
        <div className="grid items-start gap-10 xl:grid-cols-12 xl:gap-14">
          ...
          <Reveal delay={90} variant="soft" className="xl:col-span-5">
            <aside className="glance-panel rounded-2xl border border-[var(--border)] bg-surface/70 p-6 shadow-md backdrop-blur-md">
              ...
              <ul className="mt-5 space-y-4 text-sm">
                {content.glance.map((item) => (
                  <li key={item.label} className="flex gap-3">
                    <span className="min-w-[5.5rem] shrink-0 font-semibold text-foreground">
                      {item.label}
                    </span>
      ...
      <Section id="skills" eyebrow="Skills" title={content.skillsTitle}>
        <Reveal>
      ...
      <Section
        ...
        dense
      >
        <p className="-mt-4 mb-8 max-w-3xl text-pretty text-muted lg:-mt-6">
          Academic distinctions that sit alongside research, tutoring, and product delivery work.
        </p>
      ...
      <Section id="projects" eyebrow="Projects" title={content.projectsEyebrow} dense>
        ...
        <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
          {featuredManual.map((p, i) => (
            <Reveal key={`${activeMode}-${p.title}`} delay={i * 60}>
      ...
      <section className="border-t border-[var(--border)] py-12 sm:py-14">
        <div className={PAGE_CONTAINER}>
          <Reveal>
            <div className="cta-panel flex flex-col items-start justify-between gap-6 rounded-2xl border border-[var(--border)] px-8 py-10 shadow-sm sm:flex-row sm:items-center sm:px-10 lg:px-12">
              <div className="max-w-2xl">
                <h2 className="text-2xl font-semibold tracking-tight text-foreground [font-family:var(--font-display),serif] sm:text-3xl">
                  {content.ctaTitle}
                </h2>
                <p className="mt-3 text-pretty text-muted">{content.ctaBody}</p>
              </div>
              <Link
                href="/contact"
